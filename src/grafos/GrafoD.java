...
public class GrafoD {
	...
	public void coloreoM() {
		// secuencia de Matula, ordeno el vector de nodos con el de menor grado
		// primero
		Nodo[] nodos = getNodos();
		Arrays.sort(nodos);
	...
	public void coloreoWP() {
		// secuencia de Welsh-Powell, ordeno el vector de nodos con el de mayor
		// grado primero
		Nodo[] nodos = getNodos();
		Arrays.sort(nodos, Collections.reverseOrder());

		/*
		 * for (int i = 0; i < nodos.length; i++) System.out.println("Nodo " + i
		 * + ": " + nodos[i]);
		 */
		coloreo(nodos);
	}
	...
	public void tieneCiclo() throws Exception {
		Set<Integer> v = new HashSet<Integer>();
		Stack<Arista> pila = new Stack<Arista>();

		// parto desde el nodo 0
		Arista[] aristas = getAristas(0);
		v.add(0);
		...
		while (!pila.isEmpty()) {
			Arista arista = pila.pop();
			int nodoD = arista.getNodoDestino();
